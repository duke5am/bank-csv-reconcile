...
This module IS the program: it needs Python 3.8 or newer, and no third-party
libraries at all for CSV files. For .xlsx files it needs `openpyxl` (see
README.md / docs/FOR-BOOKKEEPERS.md), imported only on that one code path.

The same code is reached two ways, so the checkout and the installed package
are one implementation rather than two: `bank-csv-reconcile` (the console
script, `bank_csv_reconcile.cli:main`) and `python3 normalize.py` (a thin
wrapper at the repo root). `main()` returns an int exit code.

Run `python3 normalize.py --help` (or `bank-csv-reconcile --help`) for all
options.
...
def invocation_hint():
    """How this program was started, so the advice it prints is copy-pasteable.

    From the checkout (`python3 normalize.py ...`) this is "python3 normalize.py";
    from the installed console script it is "bank-csv-reconcile". The wrapper and
    the console script are the same code, so the hint must not hard-code either.
    """
    name = os.path.basename(sys.argv[0]) if sys.argv and sys.argv[0] else ""
    if not name or name.startswith("-"):
        return "bank-csv-reconcile"
    return ("python3 " + name) if name.endswith(".py") else name


def build_parser():
    prog = invocation_hint()
    p = argparse.ArgumentParser(
        ...
        epilog="Example:  {} statement.csv --date-order dmy\n"
               "          {} january.xlsx --out january-clean.csv "
               "--json-report january-report.json".format(prog, prog),
...
def main(argv=None):
    ...
    try:
        ...
        print("      {} \"{}\" --date-order dmy".format(
            invocation_hint(), os.path.basename(src)))
        print(RULE)
        return 2
